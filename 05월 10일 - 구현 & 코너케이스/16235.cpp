...
//아기 상어의 좌표 반환
ci findShark(int n, vector<vector<int>> &space) {
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            if (space[i][j] == 9) return ci(i, j);
        }
    }
}

//너비 우선 탐색, 이동할 곳 찾아 이동 후 추가되는 시간 반환
int bfs(int n, vector<vector<int>> &space, int size) {
    // 상, 하, 좌, 우
    int dr[] = {-1, +1, 0, 0};
    int dc[] = {0, 0, -1, +1};
    ...
    vector<vector<int>> distance(n, vector<int>(n, 0)); //아기 상어로부터 해당 칸까지의 거리 기록
    set<ci> feed_candidate; //먹이 후보 찾아서 저장, 자동 정렬

    //아기 상어의 좌표 큐에 삽입
    que.push(findShark(n, space));
    distance[que.front().first][que.front().second] = dist; //상어 자신의 거리는 0
    space[que.front().first][que.front().second] = 0; //상어 떠날 자리는 빈자리가 됨
    ...
    space[it->first][it->second] = 9; //상어 이동
